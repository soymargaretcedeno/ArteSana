...
class LocalDatabase {
    ...
    loadUsers() {
        ...
        if (storedUsers) {
            ...
            users.forEach((user) => {
                if (user && !user.phone) {
                    const phone = this.defaultPhoneFor(user);
                    if (phone) {
                        user.phone = phone;
                        changed = true;
                    }
    ...
    defaultPhoneFor(user) {
        const byId = { 1: '+507 6396-2388', 3: '+507 6234-5678', 5: '+507 6345-6789', 8: '+507 6456-7890' };
        if (user?.phone) return user.phone;
        if (byId[user?.id]) return byId[user.id];
        const name = String(user?.name || '').toLowerCase();
        if (name.includes('ana diaz') || name.includes('ana díaz')) return '+507 6396-2388';
        if (name.includes('maria gonzalez') || name.includes('maría gonzález')) return '+507 6234-5678';
        if (name.includes('isabella')) return '+507 6345-6789';
        if (name.includes('lisa')) return '+507 6456-7890';
        return '';
    }
    ...
    register(userData) {
        ...
        const newUser = {
            ...
            role: userData.role === 'artisan' ? 'artisan' : 'customer',
            roleSelected: userData.role === 'artisan' || userData.role === 'customer',
            activeMode: userData.role === 'artisan' ? 'artisan' : 'customer',
            avatar: null,
