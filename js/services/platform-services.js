...
(function (global) {
    ...
    const PlatformServices = {
        ...
        // API: GET /api/orders
        getOrders(user) {
            const stored = load(STORAGE_KEYS.orders, []);
            const fakeIds = new Set(['1042', '1038', '1031']);
            const raw = Array.isArray(stored) ? stored : [];
            const list = raw.filter((order) => {
                if (!order) return false;
                const isFakeSample = fakeIds.has(String(order.id)) && !order.items && order.userId == null;
                return !isFakeSample;
            });
            if (list.length !== raw.length) save(STORAGE_KEYS.orders, list);
            if (!user) return list.filter((order) => order.userId != null || Array.isArray(order.items));
