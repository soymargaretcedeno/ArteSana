...
(function (global) {
    ...
    function toggleFavorite(productId) {
        ...
        const isInFavorites = global.userProductsDB.isInFavorites(id);
        const heartBtn = document.querySelector(`[data-fav-id="${CSS.escape(id)}"]`);
        const heartIcon = document.getElementById(`favorite-${id}`) || heartBtn?.querySelector('.fa-heart');
        if (isInFavorites) {
            global.userProductsDB.removeFromFavorites(id);
            if (heartIcon) heartIcon.classList.remove('fav-on');
            if (heartBtn) heartBtn.classList.remove('fav-on');
            global.ecommerceConfirmation?.showFavoriteConfirmation(id, false);
        } else {
            global.userProductsDB.addToFavorites(id);
            if (heartIcon) heartIcon.classList.add('fav-on');
            if (heartBtn) heartBtn.classList.add('fav-on');
            global.ecommerceConfirmation?.showFavoriteConfirmation(id, true);
        }
    }
                ...
                <div class="product-card-explore" onclick="handleProductClick(${productIdJs(product.id)}, event)" role="button" tabindex="0" aria-label="${esc(product.name)}" onkeydown="if(event.key==='Enter')handleProductClick(${productIdJs(product.id)}, event)">
                    <div class="product-image">
                        ...
                        <div class="product-actions">
                            <button class="action-btn${inFav ? ' fav-on' : ''}" type="button" data-fav-id="${esc(product.id)}" aria-label="${esc(t('product_favorite', 'Favorito'))}" onclick="event.stopPropagation(); toggleFavorite(${productIdJs(product.id)})">
                                <i class="fas fa-heart${inFav ? ' fav-on' : ''}" id="favorite-${esc(product.id)}"></i>
                            </button>
